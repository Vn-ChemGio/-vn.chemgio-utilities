...
export function eventOrderAndStringifySubfields(obj: Audit.Event): Audit.Event {
  const orderedEntries = Object.entries(obj).sort((a, b) =>
    a[0].localeCompare(b[0]),
  );
  orderedEntries.forEach((value) => {
  ...
  return orderedObj as Audit.Event;
}
...
export function canonicalizeEvent(obj: Object) {
  return canonicalize(eventOrderAndStringifySubfields(obj as any));
}
